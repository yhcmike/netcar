...
public class AlarmUtil {
	...
	public static boolean withinRangeFence(double aLon, double aLat, FenceVo fenceVo) {
		if (null == fenceVo) {
			return false;
		}
		...
		if (fenceVo.getShape() == 0) {
			...
			// 多边形
			List<SpotVo> spotVoList = JsonUtil.extractList(fenceVo.getSpots(), SpotVo.class);
			SpotVo[] spotVoArr = new SpotVo[spotVoList.size()];
			spotVoList.toArray(spotVoArr);
			return AlarmUtil.isPtInPoly(aLon, aLat, spotVoArr);
		} else {
			return false;
		}
	 ...
	 * @return
	 */
	public static boolean isInCircle(SpotVo vo, Double[] center, Double radius) {
		double distance = DistanceUtil.distance(vo.getLng(), vo.getLat(), center[0], center[1]);
		return distance < radius;
	}

	public static boolean isInCircle(double aLon, double aLat, Double[] center, Double radius) {
